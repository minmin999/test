...
if ( ! class_exists( 'News_Unlimited_About' ) ) {
	...
	class News_Unlimited_About {
		...
		/**
		 * Singleton instance of News_Unlimited_About.
		 *
		 * @var News_Unlimited_About $instance News_Unlimited_About instance.
		 */
		private static $instance;

		/**
		 * Main News_Unlimited_About instance.
		 *
		 * @since 1.0.0
		 *
		...
		public static function init( $config ) {
			if ( ! isset( self::$instance ) && ! ( self::$instance instanceof News_Unlimited_About ) ) {
				self::$instance = new News_Unlimited_About;
		...
		public function setup_config() {
			...
			$this->notification  = isset( $this->config['notification'] ) ? $this->config['notification'] : ( '<p>' . sprintf( esc_html__( 'Welcome! Thank you for choosing %1$s! To fully take advantage of the best our theme can offer please make sure you visit our %2$swelcome page%3$s.', 'news-unlimited' ), $this->theme_name, '<a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '">', '</a>' ) . '</p><p><a href="' . esc_url( admin_url( 'themes.php?page=' . $this->page_slug ) ) . '" class="button button-primary" style="text-decoration: none;">' . sprintf( esc_html__( 'Get started with %s', 'news-unlimited' ), $this->theme_name ) . '</a><a href="' . esc_url( admin_url() . 'themes.php?status=News_Unlimited_hide_msg' ) . '" class="button" style="margin-left:5px;">' . esc_html__( 'Hide this notice', 'news-unlimited' ) . '</a></p>' );
		}
		...
		public function welcome_admin_notice() {
			...
			if( !empty( $_GET['fire-blog-theme_show_optin'] ) ){
				update_option( 'News_Unlimited_hide_msg', false );
			}

			// Hide fire blog admin message
		    if( !empty( $_GET['status'] ) && $_GET['status'] == 'News_Unlimited_hide_msg' ){
		        update_option( 'News_Unlimited_hide_msg', true );
		    }

			if ( !empty( $this->notification ) && get_option( 'News_Unlimited_hide_msg' ) != true ) {
						...
						if ( ! empty( $action_value['plugin_slug'] ) ) {
							...
							switch ( $active['needs'] ) {
								...
								case 'install':
									...
									$label = esc_html__( 'Install', 'news-unlimited' );
									break;
								case 'activate':
									$class = 'activate-now button button-primary';
									$label = esc_html__( 'Activate', 'news-unlimited' );
									break;
								case 'deactivate':
									$class = 'deactivate-now button';
									$label = esc_html__( 'Deactivate', 'news-unlimited' );
									break;
							}
							?>
		...
		public function upgrade_to_pro() {
			...
			if ( isset( $upgrade_to_pro['button_link'] ) && ! empty( $upgrade_to_pro['button_link'] ) ) {
				$button_text = esc_html__( 'Upgrade to Pro', 'news-unlimited' );
		...
		public function free_pro() {
		    ...
		    if ( ! empty( $free_pro ) ) {
		        ...
		        if ( ! empty( $free_pro ) && is_array( $free_pro ) ) {
		            ...
		            echo '<tr>';
		            echo '<th></th>';
		            echo '<th>' . esc_html__( 'News Unlimited','news-unlimited' ) . '</th>';
		            echo '<th>' . esc_html__( 'News Unlimited PRO','news-unlimited' ) . '</th>';
		            echo '</tr>';
		            echo '</thead>';
		            echo '<tbody>';
		            ...
		            echo '<td></td>';
		            echo '<td colspan="2"><a href="' . esc_url( 'https://www.templatesell.com/item/news-unlimited-pro/' ) . '" target="_blank" class="button button-primary button-hero">News Unlimited PRO</a></td>';
		            echo '</tr>';

...
$GLOBALS['smarter_thinkup_prefix'] = 'smarter_';
...
function smarter_thinkup_customizer_setup() {
...
add_action( 'customize_register', 'smarter_thinkup_customizer_setup' );

// Enqueue customizer scripts / styles
function smarter_thinkup_customizer_adminscripts() {

	global $smarter_thinkup_theme_version;

	// Add theme stylesheets.
	wp_enqueue_style( 'smarter-thinkup-customizer', get_template_directory_uri() . '/admin/main/assets/css/thinkup-customizer.css', '', $smarter_thinkup_theme_version );
	wp_enqueue_style( 'font-awesome', get_template_directory_uri() . '/lib/extentions/font-awesome/css/font-awesome.min.css', '', '4.6.3' );

	// Add theme scripts.
	wp_enqueue_script( 'smarter-thinkup-customizer', get_template_directory_uri() . '/admin/main/assets/js/thinkup-customizer.js', array( 'jquery' ), $smarter_thinkup_theme_version, true );

}
add_action( 'customize_controls_enqueue_scripts', 'smarter_thinkup_customizer_adminscripts' );

// Assign custom function to fetch variable values
if ( !function_exists( 'smarter_thinkup_var' ) ) {
	function smarter_thinkup_var( $name, $key = false ) {

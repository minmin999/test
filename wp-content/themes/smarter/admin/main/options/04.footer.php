...
function smarter_thinkup_input_footerw1() {
	echo	'<div id="footer-col1" class="widget-area">';
	if ( ! dynamic_sidebar( 'footer-w1' ) and current_user_can( 'edit_theme_options' ) ) {
		echo	'<h3 class="widget-title">' . __( 'Please Add Widgets', 'smarter') . '</h3>',
			'<div class="error-icon">',
				'<p>' . __( 'Remove this message by adding widgets to Footer Widget Area 1.', 'smarter') . '</p>',
				'<a href="' . esc_url( admin_url( 'widgets.php' ) ) . '" title="' . esc_attr__( 'No Widgets Selected', 'smarter' ) . '">' . __( 'Click here to go to Widget area.', 'smarter') . '</a>',
			'</div>';
	};
	echo	'</div>';
}
...
function smarter_thinkup_input_footerw2() {
	echo	'<div id="footer-col2" class="widget-area">';
	if ( ! dynamic_sidebar( 'footer-w2' ) and current_user_can( 'edit_theme_options' ) ) {
	echo	'<h3 class="widget-title">' . __( 'Please Add Widgets', 'smarter') . '</h3>',
			'<div class="error-icon">',
				'<p>' . __( 'Remove this message by adding widgets to Footer Widget Area 2.', 'smarter') . '</p>',
				'<a href="' . esc_url( admin_url( 'widgets.php' ) ) . '" title="' . esc_attr__( 'No Widgets Selected', 'smarter' ) . '">' . __( 'Click here to go to Widget area.', 'smarter') . '</a>',
			'</div>';
	};
	echo	'</div>';
...
function smarter_thinkup_input_footerw3() {
	echo	'<div id="footer-col3" class="widget-area">';
	if ( ! dynamic_sidebar( 'footer-w3' ) and current_user_can( 'edit_theme_options' ) ) {
	echo	'<h3 class="widget-title">' . __( 'Please Add Widgets', 'smarter') . '</h3>',
			'<div class="error-icon">',
				'<p>' . __( 'Remove this message by adding widgets to Footer Widget Area 3.', 'smarter') . '</p>',
				'<a href="' . esc_url( admin_url( 'widgets.php' ) ) . '" title="' . esc_attr__( 'No Widgets Selected', 'smarter' ) . '">' . __( 'Click here to go to Widget area.', 'smarter') . '</a>',
			'</div>';
	};	
	echo	'</div>';
...
function smarter_thinkup_input_footerw4() {
	echo	'<div id="footer-col4" class="widget-area">';
	if ( ! dynamic_sidebar( 'footer-w4' ) and current_user_can( 'edit_theme_options' ) ) {
	echo	'<h3 class="widget-title">' . __( 'Please Add Widgets', 'smarter') . '</h3>',
			'<div class="error-icon">',
				'<p>' . __( 'Remove this message by adding widgets to Footer Widget Area 4.', 'smarter') . '</p>',
				'<a href="' . esc_url( admin_url( 'widgets.php' ) ) . '" title="' . esc_attr__( 'No Widgets Selected', 'smarter' ) . '">' . __( 'Click here to go to Widget area.', 'smarter') . '</a>',
			'</div>';	
	};	
	echo	'</div>';
...
function smarter_thinkup_input_footerw5() {
	echo	'<div id="footer-col5" class="widget-area">';
	if ( ! dynamic_sidebar( 'footer-w5' ) and current_user_can( 'edit_theme_options' ) ) {
	echo	'<h3 class="widget-title">' . __( 'Please Add Widgets', 'smarter') . '</h3>',
			'<div class="error-icon">',
				'<p>' . __( 'Remove this message by adding widgets to Footer Widget Area 5.', 'smarter') . '</p>',
				'<a href="' . esc_url( admin_url( 'widgets.php' ) ) . '" title="' . esc_attr__( 'No Widgets Selected', 'smarter' ) . '">' . __( 'Click here to go to Widget area.', 'smarter') . '</a>',
			'</div>';	
	};	
	echo	'</div>';
...
function smarter_thinkup_input_footerw6() {
	echo	'<div id="footer-col6" class="widget-area">';
	if ( ! dynamic_sidebar( 'footer-w6' ) and current_user_can( 'edit_theme_options' ) ) {
	echo	'<h3 class="widget-title">' . __( 'Please Add Widgets', 'smarter') . '</h3>',
			'<div class="error-icon">',
				'<p>' . __( 'Remove this message by adding widgets to Footer Widget Area 6.', 'smarter') . '</p>',
				'<a href="' . esc_url( admin_url( 'widgets.php' ) ) . '" title="' . esc_attr__( 'No Widgets Selected', 'smarter' ) . '">' . __( 'Click here to go to Widget area.', 'smarter') . '</a>',
			'</div>';	
	};	
	echo	'</div>';
...
function smarter_thinkup_input_footerlayout() {	

// Get theme options values.
$thinkup_footer_layout       = smarter_thinkup_var ( 'thinkup_footer_layout' );
$thinkup_footer_widgetswitch = smarter_thinkup_var ( 'thinkup_footer_widgetswitch' );
		...
		echo	'<div id="footer">';
			if ( $thinkup_footer_layout == "option1" ) {
				...
				echo	'<div id="footer-core" class="option4">';
						...
						smarter_thinkup_input_footerw2();
						smarter_thinkup_input_footerw3();
						smarter_thinkup_input_footerw4();
				echo	'</div>';
				...
				echo	'<div id="footer-core" class="option5">';
						...
						smarter_thinkup_input_footerw3();
						smarter_thinkup_input_footerw4();
						smarter_thinkup_input_footerw5();
				echo	'</div>';
				...
				echo	'<div id="footer-core" class="option6">';
						...
						smarter_thinkup_input_footerw4();
						smarter_thinkup_input_footerw5();
						smarter_thinkup_input_footerw6();
				echo	'</div>';
...
function smarter_thinkup_input_copyright() {

	printf( __( 'Developed by %1$s. Powered by %2$s.', 'smarter' ) , '<a href="//www.thinkupthemes.com/" target="_blank">Think Up Themes Ltd</a>', '<a href="//www.wordpress.org/" target="_blank">WordPress</a>'); 
}
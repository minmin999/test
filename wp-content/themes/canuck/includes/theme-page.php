...
function canuck_load_theme_page() {
	if ( isset( $_POST['canuck-subpage-submit'] ) ) { // WPCS: input var ok.
		$url_parameters = isset( $_GET['tab'] ) ? 'updated=true&tab=' . esc_attr( wp_unslash( $_GET['tab'] ) ) : 'updated=true'; // WPCS: input var ok, sanitization ok.
...
function canuck_theme_page_callback() {
	...
	<div class="canuck-wrap">
		...
		<?php
		if ( isset( $_GET['tab'] ) ) {// WPCS: input var ok.
		...
		<div id="canuck-page">
			<?php
			if ( isset( $_GET['page'] ) ) {
				$page_string = esc_attr( wp_unslash( $_GET['page'] ) );
			} else {
				$page_string = '';
			}
...
function canuck_intro_setup_callback() {
	...
	<div class="katb_paypal"><?php esc_html_e( 'Show your appreciation!', 'canuck' ); ?>
		...
		<a href="https://kevinsspace.ca/canuckdemo/donate/" target="_blank">
			<img alt="<?php esc_attr_e( 'Donate Button with Credit Cards', 'canuck' ); ?>" src="https://www.paypalobjects.com/webstatic/en_US/btn/btn_donate_cc_147x47.png" />
		</a>
	</div>
	<p>
		<?php
		echo esc_html__( 'Author Site : ', 'canuck' ) . '<a href="//kevinsspace.ca" target="_blank" >kevinsspace.ca</a>&nbsp;&nbsp;&nbsp;';
		echo esc_html__( 'Demo Site : ', 'canuck' ) . '<a href="//cauckdemo.kevinsspace.ca/" target="_blank" >Canuck Demo</a>';
		?>
	</p>
	<?php

...
add_action( 'wp', 'dollah_woo_setup' );
 ...
 * @return void
 */
function dollah_woo_register_sidebars() {
	hybrid_register_sidebar(
		array(
			'id'          => 'hoot-woocommerce-sidebar',
			'name'        => _x( 'Woocommerce Primary Sidebar', 'sidebar', 'dollah' ),
			'description' => __( 'The primary sidebar for woocommerce pages.', 'dollah' )
		)
	);
	hybrid_register_sidebar(
		array(
			'id'          => 'hoot-woocommerce-sidebar-secondary',
			'name'        => _x( 'Woocommerce Secondary Sidebar', 'sidebar', 'dollah' ),
			'description' => __( 'The secondary sidebar for woocommerce pages (if you are using a 3 column layout with 2 sidebars).', 'dollah' )
		)
	);
}
add_action( 'widgets_init', 'dollah_woo_register_sidebars' );
 ...
 * @return array
 */
function dollah_theme_woo_attr_sidebar( $attr, $context ) {
...
add_filter( 'hybrid_attr_sidebar', 'dollah_theme_woo_attr_sidebar', 11, 2 );
 ...
 * @return array
 */
function dollah_woo_main_layout( $sidebar ) {
	...
	if ( is_woocommerce() ){
		if ( is_product() ) { // single product page. Wrapper for is_singular
			$sidebar = dollah_get_mod( 'sidebar_wooproduct' );
		} else { // shop, category, tag archives etc
			$sidebar = dollah_get_mod( 'sidebar_wooshop' );
		}
	}

	// Let developers edit default layout for Cart and Checkout which are standard 'Pages' with shortcodes
	$forcenosidebar = apply_filters( 'dollah_woo_pages_force_nosidebar', true );
...
add_filter( 'dollah_main_layout', 'dollah_woo_main_layout' );
 ...
 * @param string $context
 */
if ( !function_exists('dollah_woo_meta_info_blocks_display') ) {
function dollah_woo_meta_info_blocks_display( $display, $context ) {
	if ( is_woocommerce() || is_cart() || is_checkout() || is_account_page() )
		$display = array();
	return $display;
}
}
add_filter( 'dollah_meta_info_blocks_display', 'dollah_woo_meta_info_blocks_display', 10, 2 );
 ...
 * @return int
 */
if ( !function_exists('dollah_woo_loop_per_page') ) {
function dollah_woo_loop_per_page( $value ) {
	return intval( dollah_get_mod( 'wooshop_products' ) );
}
}
add_filter( 'loop_shop_per_page', 'dollah_woo_loop_per_page', 20 );
 ...
 * @return int
 */
if ( !function_exists('dollah_woo_loop_columns') ) {
function dollah_woo_loop_columns( $value ) {
	return intval( dollah_get_mod( 'wooshop_product_columns' ) );
}
}
add_filter( 'loop_shop_columns', 'dollah_woo_loop_columns', 999 );
 ...
 * @return void
 */
if ( !function_exists('dollah_woo_custom_loop_columns_css') ) {
function dollah_woo_custom_loop_columns_css() {
	$columns = dollah_get_mod( 'wooshop_product_columns' );
...
add_action( 'wp_enqueue_scripts', 'dollah_woo_custom_loop_columns_css', 99 );
 ...
 * @return void
 */
function dollah_woo_set_queried_object( $q ){
	if ( $q->is_main_query() )
		$r = get_queried_object();
}
add_action( 'pre_get_posts', 'dollah_woo_set_queried_object', 9 );
 ...
 * @return void
 */
if ( !function_exists( 'dollah_hide_loop_meta_woo_product' ) ) :
function dollah_hide_loop_meta_woo_product() {

<?php
global $dollah_theme, $hybridextend_style_builder;
...
do_action( 'dollah_slider_start', 'html' );
...
if ( isset( $dollah_theme->sliderSettings ) && is_array( $dollah_theme->sliderSettings ) ) {
	...
	if ( !empty( $dollah_theme->sliderSettings['class'] ) )
		$class .= ' ' . sanitize_html_class( $dollah_theme->sliderSettings['class'] );

	if ( !empty( $dollah_theme->sliderSettings['id'] ) )
	...
	if ( !empty( $dollah_theme->sliderSettings['min_height'] ) ) {
		// use height instead of min-height (firefox) http://stackoverflow.com/questions/7790222/
		$gridstylearray = $hybridextend_style_builder->css_rule_sanitized_array( 'height', $dollah_theme->sliderSettings['min_height'] . 'px' );
...
<ul class="lightSlider<?php echo $class; ?>"<?php echo $atts; ?>><?php
	foreach ( $dollah_theme->slider as $key => $slide ) :
		...
		$slide['button'] = empty( $slide['button'] ) ? __('Know More', 'dollah') : $slide['button'];
				...
				<div class="hgrid"<?php if ( !empty( $gridstyle ) ) echo ' style="' . esc_attr( $gridstyle ) . '"'; ?>>
					...
					<?php if ( !empty( $slide['image'] ) ) { ?>
						<div class="<?php echo $column; ?> hootslider-html-slide-column hootslider-html-slide-right">
							...
							if ( !empty( $imageid ) )
								echo wp_get_attachment_image( $imageid, apply_filters( 'dollah_htmlslider_imgsize', 'full', '', array( 'class' => 'hootslider-html-slide-img' ) ) );
							else

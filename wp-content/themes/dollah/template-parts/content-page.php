...
?>
	...
	<article <?php hybridextend_attr( 'page' ); ?>>
		...
		<?php
		$hide_meta_info = apply_filters( 'dollah_hide_meta_info', false, 'bottom' );
		if ( !$hide_meta_info && 'bottom' == dollah_get_mod( 'post_meta_location' ) ):
			$metarray = dollah_get_mod('page_meta');
			if ( dollah_meta_info_display( $metarray, 'page', true ) ) :
			?>
			<footer class="entry-footer">
				<?php dollah_meta_info_blocks( $metarray, 'page' ); ?>
			</footer><!-- .entry-footer -->
			<?php
			endif;
		endif;
 ...
 */
	...
	global $dollah_theme;
	if ( empty( $dollah_theme->searchresults_hide_pages ) ) {

		$archive_type = apply_filters( 'dollah_default_archive_type', 'block2', 'content-page' );
		$archive_template = apply_filters( 'dollah_default_archive_location', 'template-parts/content-archive', $archive_type, 'content-page' );

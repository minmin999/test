...
function hybridextend_get_excerpt( $words ) {
	if ( empty( $words ) ) {
		...
		global $dollah_theme;
		$dollah_theme->excerpt_customlength = $words;
		...
		unset( $dollah_theme->excerpt_customlength );
		return $return;
	}
}
...
function hybridextend_getexcerpt_customlength( $length ){
	global $dollah_theme;
	if ( !empty( $dollah_theme->excerpt_customlength ) )
		return $dollah_theme->excerpt_customlength;
	else
		return $length;
}
...
function hybridextend_locate_uri( $location, $type, $return = 'uri' ) {
	...
	elseif ( defined( 'HYBRIDEXTEND_DEBUG' ) )
		...
		$loadminified = dollah_get_mod( 'load_minified', 0 );
...
function hybridextend_get_image_size_name( $span, $crop=NULL, $gridadjust=0 ) {
	...
	// $grid = ( function_exists( 'dollah_get_mod' ) ) ? intval( dollah_get_mod( 'site_width' ) ) : 0;
	// if ( empty( $grid ) )
		$grid = $default_grid;
	$grid -= $gridadjust;

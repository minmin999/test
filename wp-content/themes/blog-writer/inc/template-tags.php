...
if ( ! function_exists( 'blog_writer_posted_on' ) ) :
	function blog_writer_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( 'Posted %s', 'post date', 'blog-writer' ),
...
function blog_writer_categories() {
		...
		if ( $categories_list ) {
			...
			printf( '<li class="cat-links">' . esc_html__( 'In %1$s', 'blog-writer' ) . '</li>', $categories_list ); // WPCS: XSS OK.
		}
	}
endif;
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'blog-writer' ),
						array(
							'span' => array(
								'class' => array(),
							),
...
if ( ! function_exists( 'blog_writer_entry_footer' ) ) :
	...
	function blog_writer_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {			
			// Get tag list
			if(get_the_tag_list()) {
				echo '<p id="tag-list-title">' . esc_html__( 'Posts Tagged with&hellip;', 'blog-writer' ) . '</p>';
...
// Edit link function
if ( ! function_exists( 'blog_writer_edit_link' ) ) :
	function blog_writer_edit_link() {
		edit_post_link(
			sprintf(
				wp_kses(
					/* translators: %s: Name of current post. Only visible to screen readers */
					__( 'Edit <span class="screen-reader-text">%s</span>', 'blog-writer' ),
					array(
						'span' => array(
							'class' => array(),
						),
